...
namespace ExamPreparation6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var plates = Console.ReadLine().Split(new[] { " " },
                StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
            Queue<int> platesOfTheAragornsDefence = new Queue<int>(plates);
            ...
            {
                var newWave = Console.ReadLine().Split(new[] { " " }, 
                    StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
                Stack<int> newWaveOrcs = new Stack<int>(newWave);
